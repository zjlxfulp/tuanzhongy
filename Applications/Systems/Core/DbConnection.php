...
class DbConnection
{
    ...
    public function __construct($db, $app_name, $single)
    {
        $this->link = $db;
    ...
    {
        $configName = $this->app_name.'\Config\Db';
//        $this->distributed = call_user_func_array(array( $this->app_name.'\Config\Db', 'getItem'), array('distributed'));
        $this->distributed = $configName::$distributed;

        $qtype = strtolower($this->type);
        if($this->distributed == true && $this->single == false) {
            ...
            $dbs = $configName::$$db;
            $randKey = array_rand($dbs);
            $this->link = $dbs[$randKey];
        }

        $this->settings = $configName::getConfig($this->link);
        $this->connectreal();
    }
     ...
     */
    protected function connectreal()
    {
        if(isset(self::$pdos[$this->link])) {
            $this->pdo = self::$pdos[$this->link];
        } else {
            $dsn       = 'mysql:dbname=' . $this->settings["dbname"] . ';host=' .
            $this->settings["host"] . ';port=' . $this->settings['port'];

            $this->pdo =self::$pdos[$this->link] =  new PDO($dsn, $this->settings["user"], $this->settings["password"],
                array(
...
//            $msg ="错误原因：".$e->getMessage()."|";
//            $msg.="错误文件：".$e->getFile()."|";
//            $msg.="错误代码：".$e->getCode()."|";
//            $msg.="行数：".$e->getLine();
            echo $msg ="错误原因：".$e->getMessage();
//            file_put_contents("/opt/log/danche_error_sql.log",date('Y-m-d H:i:s').":". $query."[ error : ".$msg."]"."\n\n",FILE_APPEND);
            // 服务端断开时重连一次
    ...
    public function query($query = '', $params = null, $fetchmode = PDO::FETCH_ASSOC)
    {
        ...
        $this->type = $statement = strtolower(trim($rawStatement[0]));
	...
	public function showAllFields($table) {
		$this->resetAll();
		$this->sQuery = $this->pdo->prepare("DESCRIBE $table");
		$this->sQuery->execute();
