...
class Redis
{
     ...
     * @param int $post
     */
	public function __construct($app_name, $single, $num = 0) {
        $this->redis = new \Redis();

        $configName = $app_name.'\Config\Redis';
        $this->distributed = $configName::$distributed;

        if($this->distributed == true) {
            $dbs = $configName::$servers;
            $randKey = array_rand($dbs);
            $link = $dbs[$randKey];
        } else {
            $link = $configName::$singleServer;
        }

        $config = $configName::getConfig($link);;
