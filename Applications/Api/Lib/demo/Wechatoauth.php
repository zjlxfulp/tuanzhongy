...
{
    //这个号是我自己的测试号
    const WECHAT_APPID = 'wxfa03dbf605018a45';
    const WECHAT_APPSECRET = 'd4624c36b6795d1d99dcf0547af5443d';
    const WECHAT_URL = 'http://t.bigfacetech.com:5000/';
    ...
    public static function getCode($redirect_uri, $state=1, $scope='snsapi_base'){
        ...
        $redirect_uri = self::WECHAT_URL . $redirect_uri;
        ...
        Http::header('Location: '.$url, true, 301);
    }
    ...
    public static function getAccessTokenAndOpenId($code){
        ...
        //构造请求微信接口的URL
        $url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid='.self::WECHAT_APPID.'&secret='.self::WECHAT_APPSECRET.'&code='.$code.'&grant_type='.$grant_type.'';
        //请求微信接口, Array(access_token, expires_in, refresh_token, openid, scope)
        Curl::Init();
        $json = json_decode(Curl::get($url), true);
        Curl::close();

        if(isset($json['errcode'])) {
            throw new SException('', 600, $json['errmsg']);
        }
    ...
    public static function refreshToken($refreshToken){
        $url = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.self::WECHAT_APPID.'&grant_type=refresh_token&refresh_token='.$refreshToken;
        Curl::Init();
        $json = json_decode(Curl::get($url), true);
        Curl::close();

        if(isset($json['errcode'])) {
            throw new SException('', 601, $json['errmsg']);
        }
    ...
    public static function getUserInfo($accessToken, $openId, $lang='zh_CN'){
        ...
        if(isset($json['errcode'])) {
            throw new SException('', 602, $json['errmsg']);
        }
    ...
    public static function checkAccessToken($accessToken, $openId){
        ...
        if(isset($json['errcode'])) {
            throw new SException('', 603, $json['errmsg']);
        }
