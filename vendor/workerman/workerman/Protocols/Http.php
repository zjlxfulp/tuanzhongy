...
class Http
{
    ...
    {
        $config = call_user_func(array( 'Common\Config\Session', 'getConf'));
        if (PHP_SAPI != 'cli') {
            return session_start();
        }
        self::tryGcSessions($config);
        ...
        if (!isset($_COOKIE[HttpCache::$sessionName])) {
            ...
            $session_id = $_COOKIE[HttpCache::$sessionName];
        }
        $time_now  = time();
        switch ($config['driver']) {
            case 'db'://数据库存储
                ...
                if($flag == true) {
                    ...
                    $sesData =   $db->select('*')
                                    ->from('sessions')
                                    ->where("id = '{$session_id}'")
                                    ->where("timestamp > $time_now")
                                    ->row();

                    if(empty($sesData)) {
                        $flag = false;
                    } else {
                        $_SESSION = json_decode($sesData['data'], true);
                        return true;
                    }
                }
                break;
            default ://默认是文件
                if($flag == true) {
                    if (is_file(HttpCache::$sessionPath . '/ses' . $session_id)) {
                        if (!HttpCache::$instance->sessionFile) {
                            HttpCache::$instance->sessionFile = HttpCache::$sessionPath . '/ses' . $session_id;
                        }
            ...
            if($config['driver'] == 'db') {
                $db->insert('sessions')
                    ->cols(
                        array(
                            'id' => $session_id,
                            'timestamp' => time() + $config['cookie_lifetime'],
                            'data' => '',
                        )
                    )
                    ->query();
     ...
     * @return void
     */
    public static function tryGcSessions( & $config)
    {
        ...
        switch ($config['driver']) {
            case 'db'://数据库存储
                $db = Db::instance(true);
                $sessions = $db->select('id')->from('sessions')->where("timestamp <= $time_now")->query();
                if(!empty($sessions)) {
                    foreach ($sessions as &$val) {
                        $val = "'{$val['id']}'";
                    }
                    $sessions = implode(',', $sessions);
                    $db->delete('sessions')->where("id in ({$sessions})")->query();
                }
                break;
            default ://默认是文件
                foreach(glob(HttpCache::$sessionPath.'/ses*') as $file) {
                    if(is_file($file) && $time_now - filemtime($file) > $config['cookie_lifetime']) {
                        unlink($file);
                    }
                }
                break;
